...
const getBrokerColor = (code: string) => {
  ...
  const overrides: Record<string, string> = {
    'XL': 'hsla(345, 85%, 82%, 0.45)',
    'CC': 'hsla(145, 85%, 82%, 0.45)',
    'ZP': 'hsla(265, 85%, 82%, 0.45)',
    'AK': 'hsla(200, 85%, 82%, 0.45)',
    'MG': 'hsla(25, 85%, 82%, 0.45)',
    'YP': 'hsla(180, 85%, 82%, 0.45)',
    'PD': 'hsla(55, 85%, 82%, 0.45)',
    'AZ': 'hsla(100, 85%, 82%, 0.45)',
  };
...
export default function PerformancePage() {
  ...
  const fetchPerformance = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/user-performance?${params}`);
  ...
  if (!user) {
    return (
      <div className="container" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
        <ConnectRequiredCard message="Silakan Connect Google untuk melihat performa analisis Anda." />
      </div>
    );
  }
  ...
  return (
    <div className="container" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
      ...
      <p style={{ color: 'var(--text-muted)', marginBottom: '1.25rem', fontSize: '0.9rem' }}>
        Tingkat keakuratan target Adimology berdasarkan riwayat analisis {isAdmin ? 'semua pengguna' : 'Anda'}.
      </p>
                  ...
                  {data.length === 0 && (
                    <tr>
                      <td colSpan={10} style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                        Belum ada data performa untuk periode ini. Lakukan analisis menggunakan kalkulator untuk mulai melacak performa Anda.
                      </td>
                    </tr>
                  )}
                </tbody>
